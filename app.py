...
<form id="orderForm" class="wrap" onsubmit="return submitOrder(event)">
  ...
  <div class="card">
    ...
    <label class="field">Room Number
      <input type="text" name="room_number" maxlength="20" placeholder="e.g. 302" />
    </label>
    <label class="field">Contact / Phone
      <input type="text" name="phone" maxlength="40" placeholder="Optional" />
    </label>
    <label class="field">Notes / Special Requests
      <textarea name="notes" placeholder="Allergies, less spicy, etc."></textarea>
    </label>
  </div>
...
<div class="wrap">
  ...
  <table>
    <thead>
      <tr>
        <th>#</th><th>Time</th><th>Customer</th><th>Room</th><th>Phone</th>
        <th>Items Ordered</th><th>Qty</th><th>Notes</th>
      </tr>
    </thead>
    <tbody>
      {% for o in orders %}
...
def orders_csv():
    ...
    buf = io.StringIO()
    w = csv.writer(buf)
    w.writerow(["Order ID","Time","Customer","Room","Phone","Item Count","Items","Notes"])
    for r in rows:
        items = json.loads(r["items_json"])
        items_str = "; ".join(f"{i['name']} x{i['quantity']} ({i['category']})" for i in items)
        w.writerow([r["id"], r["created_at"], r["customer_name"], r["room_number"] or "",
                    r["phone"] or "", r["item_count"], items_str, r["notes"] or ""])
